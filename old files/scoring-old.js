  ...
  generateRecommendations(gameState, results) {
    ...
    if (profile.coaching < 30) {
      recommendations.push({
        ...
        description: `Current usage: ${profile.coaching}% | Optimal: 35-40%. Recommended: Read "The Coaching Habit" by Michael Bungay Stanier. Practice: Spend 30 min weekly in development conversations. Why: Your team needs growth.`
      });
    }
    ...
    const shortTermDecisions = gameState.decisions.filter(d => d.scenario.id && d.scenario.id < 2 // Early scenarios
    ).length;

    if (shortTermDecisions > 9) {
      recommendations.push({
